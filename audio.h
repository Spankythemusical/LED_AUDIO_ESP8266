...
byte beatDetect() {
  ...
  float specCombo = spectrumValue[6]; //(spectrumValue[0] + spectrumValue[6]) / 2.0;
  beatAvg = AGCSMOOTH * specCombo; //(1.0 - AGCSMOOTH) * beatAvg + AGCSMOOTH * specCombo;
    if (specCombo >= 250 && specCombo <= 400) {
    beatTriggered = 1;
    return 1;
  } else if (specCombo < 250) {
    beatTriggered = 0;
    return 0;
  } else if (specCombo > 400) {
...
{
  fill_solid(leds, NUM_LEDS, CRGB::Green); //CRGB::Black);
  ...
  for (uint16_t i = 0; i < 7; i++) {
    ...
    uint16_t columnHeight = (map8(spectrumByte[i], 1, columnSize)) * 2.5;
...
void analyzerPeakColumns()
{
  ...
  for (uint16_t i = 0; i < 7; i++) {
    uint16_t columnStart = i * columnSize;
    uint16_t columnEnd = (columnStart + columnSize) * 3;

    if (columnEnd >= NUM_LEDS) columnEnd = NUM_LEDS - 1;

    uint16_t columnHeight = map(spectrumValue[i], 0, 1023, 0, columnSize);
    uint16_t peakHeight = map(spectrumPeaks[i], 0, 1023, 0, columnSize);
...
void beatWaves()
{
  ...
  fade_down(1);
  if (beatDetect()) {
    //CRGB color = ColorFromPalette(gCurrentPalette, spectrumByte[6], spectrumByte[6]);
    CRGB color = ColorFromPalette(palettes[currentPaletteIndex], gHue);
    leds[CENTER_LED] = color;
  }

  //move to the left
  for (uint16_t i = NUM_LEDS - 1; i > CENTER_LED; i--) {
    leds[i] = leds[i - 1];
  }
  // move to the right
  for (uint16_t i = 0; i < CENTER_LED; i++) {
    leds[i] = leds[i + 1];
  }
}
...
void drawVU() {
  ...
  const int xScale = 255.0 / (NUM_LEDS / 2);
  uint16_t specCombo = (spectrumDecay[0] + spectrumDecay[1] + spectrumDecay[5] + spectrumDecay[6]) / 4;

  for (uint16_t x = 0; x < NUM_LEDS / 2; x++) {
    Serial.println(specCombo);
...
void drawVU2() {
  if (spectrumAvg >= 40 && spectrumAvg < 100) {
    spectrumAvg = (spectrumAvg * 6);
  } else if (spectrumAvg >= 20 && spectrumAvg < 40) {
    spectrumAvg = (spectrumAvg * 8.5);
  } else if (spectrumAvg >= 1 && spectrumAvg < 20) {
    spectrumAvg = (spectrumAvg * 9.5);
  } else if (spectrumAvg = 0) {
    spectrumAvg = 185;
  } else {
    spectrumAvg = spectrumAvg;
  }
  uint16_t var = map8(NUM_LEDS - 1, 0, spectrumAvg);
  uint16_t avg = var * 80;
  for (uint16_t i = 0; i < NUM_LEDS; i++) {

    if (i <= avg) {
      leds[i] = ColorFromPalette(palettes[currentPaletteIndex], (NUM_LEDS / 240) * i);
    }
    else {
      leds[i] = ColorFromPalette(palettes[currentPaletteIndex], gHue, LINEARBLEND); //CRGB::Black;
